...
def connection_options(f):
    ...
    """
    f = click.option('-t', '--timeout', type=click.INT, default=10, show_default=True, metavar='SECONDS',
                     help='Timeout in seconds against Elasticsearch.')(f)
    f = click.option('-u', '--user', type=click.STRING, metavar='NAME',
                     help='User for Elasticsearch basic auth. Prompts for password.')(f)
    f = click.option('-C', '--ca-certificate', type=click.Path(exists=True, dir_okay=False, readable=True),
                     help='Use TLS with your self-signed CA certificate.')(f)
    f = click.option('-T', '--tls', type=click.BOOL, is_flag=True,
                     help='Enable TLS against Elasticsearch.')(f)
    f = click.option('-s', '--sites', type=click.STRING, metavar='NAME', help='Sites to connect to.')(f)
    f = click.option('-c', '--cluster', type=click.STRING, metavar='NAME', help='Cluster to connect to.')(f)
    f = click.option('-H', '--host', type=click.STRING, metavar='NAME', help='Specific host to connect to.')(f)

    @functools.wraps(f)
    def decorator(sites, cluster, host, user, ca_certificate, tls, timeout, *args, **kwargs):
        ctx = click.get_current_context()
        if user:
            password_option = click.prompt(f'Password for user "{user}"', hide_input=True)
        else:
            password_option = None

        ctx.meta[_CONNECTIONS_KEY] = dict(host_option=host, cluster_option=cluster, sites_option=sites,
                                          user_option=user, password_option=password_option,
                                          ca_certificate_option=ca_certificate, tls_option=tls, timeout_option=timeout)
...
def per_connection(include_site=False):
    def wrapper(f):
        ...
        def decorator(*args, **kwargs):
            ...
            if _CONNECTIONS_KEY not in ctx.meta or ctx.obj is None or 'config' not in ctx.obj:
                LOG.error('Connection options mismatch. This is a bug. Would you please file a bug report? Thanks!')
                sys.exit(CLI_ERROR)
            ...
            for client, site, cluster in clients:
                if len(clients) > 1:
                    click.secho('{} - {}:'.format(cluster, site), bold=True, underline=True)
...
def resolve_remote(remote, site):
    ...
    if site is None and '{site}' in config['cluster_hostname_pattern']:
        ...
        config = config.copy().update(host_option=None, cluster_option=remote, sites_option=site)
...
def _create_clients(config):
    ...
    if host_option is not None:
        ...
        if config['cluster_hostname_pattern'] is None:
            LOG.error('Neither cluster section or hostname pattern is defined for specified cluster. '
        ...
        clients = _cluster_clients(connections, default_connection, sites_option, config)

    else:
        # No connection info given, fallback to default host.
        if sites_option:
            LOG.error('--sites specified, but default connection is not a predefined connection. '
...
def _cluster_clients(connections, cluster_option, sites_option, config):
    ...
    for site in desired_sites:
        ...
        if hostname is None:
            LOG.error('The cluster "{cluster}" has no defined site "{site}".\n'
...
def _cluster_sites(connections, sites_option, cluster_option):
    ...
    return _parse_sites(sites_option) if sites_option else connections[cluster_option].keys()
...
def _patterned_hostname(config, cluster_option, sites_option):
    ...
    if '{cluster}' not in cluster_hostname_pattern:
        LOG.error('"cluster_hostname_pattern" must contain the "{cluster}" variable. '
                  'Check your config file.')
        sys.exit(CONFIGURATION_ERROR)

    if sites_option is not None:
        if '{site}' not in cluster_hostname_pattern:
            LOG.error('Site option is specified, but "{site}" variable is missing in "cluster_hostname_pattern". '
                      'Check your config file.')
            sys.exit(USER_ERROR)
        ...
        if '{site}' not in cluster_hostname_pattern and default_sites is not None:
            LOG.error('"cluster_pattern_default_sites" are defined, but "{site}" variable is missing in '
    ...
    try:
        if sites is not None:
            for site in sites:
                client = _make_client(cluster_hostname_pattern.format(cluster=cluster_option, site=site), config)
                clients.append((client, site, cluster_option))
        else:
            client = _make_client(cluster_hostname_pattern.format(cluster=cluster_option), config)
            clients.append((client, None, cluster_option))

    except KeyError:
        LOG.exception('Could not parse cluster hostname pattern. Pattern must include "{cluster}" variable, for usage '
...
def _make_client(hostname, config):
    ...
    try:
        ...
        LOG.exception('Could not create Elasticsearch client for given hostname: {}'.format(hostname))
        sys.exit(USER_ERROR)
