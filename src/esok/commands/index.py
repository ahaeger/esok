...
@click.option('-R', '--remote', type=click.STRING, metavar='HOSTNAME',
...
@click.option('-m', '--skip-mapping', is_flag=True, help='Skip the mapping part of the index when copying.')
@click.option('-s', '--skip-settings', is_flag=True, help='Skip the settings part of the index when copying.')
...
def copy(client, site, source_index, new_index, remote, skip_mapping, skip_settings):
    ...
    LOG.info(json.dumps(r))
    if not ack:
        LOG.warning('Request timed out before acknowledge was received. Index might not have been created yet.')
...
@click.option('-n', '--nodes', type=click.INT, help='Manually set node count of the cluster.')
@per_connection()
def shards(client, name, shard_count, absolute, nodes):
    ...
    if absolute:
        ...
        if not nodes:
            # Fetch node count.
            r = client.nodes.stats(metric='process')  # Using this metric because payload is small
            cluster_node_count = r.get('nodes').values()
            # Figure out how many data nodes there are in this cluster
            data_node_count = sum(1 for item in cluster_node_count if 'data' in item.get('roles'))
            LOG.debug('Resolved data node count from cluster: {}'.format(data_node_count))
        else:
        ...
        r = client.indices.get(index=name)
        primary_count = int(r.get(name).get('settings').get('index').get('number_of_shards'))
        ...
        replica_count = max(replica_count, 0.0)  # For the edge case where replica_count == -1

        if not replica_count.is_integer():
            click.echo('The cluster configuration and desired shards per machine resulted in {} total replicas.\n'
                       'This will be rounded to {} replicas in total.'.format(replica_count, int(replica_count)))
    ...
    body = {
        'index': {
            'number_of_replicas': int(replica_count)
        }
    }
...
@index.command()
@click.argument('name', type=click.STRING)
@click.option('-o', '--output-file', type=click.File('w'), default='-', show_default=True,
              help='Specify file to output to.')
@click.option('-c', '--chunk-size', type=click.INT, default=1000, show_default=True,
              help='Number of documents (per shard) to fetch in each individual request.')
@click.option('-s', '--scroll-time', type=click.STRING, default='5m', show_default=True,
...
@click.option('-i', '--index-name', type=click.STRING, help='Forces target index to given name for all documents.')
@click.option('-c', '--chunk-size', type=click.INT, help='Number of docs in one chunk', default=500,
              show_default=True)
@click.option('--refresh', is_flag=True, help='Refresh the index after indexing.')
@click.option('-C', '--max-chunk-bytes', type=click.INT, help='The maximum size of the request in bytes',
              default=int(100e6), show_default=True)
@click.option('-R', '--max-retries', type=click.INT,
              help='Maximum number of times a document will be retried when 429 is received', default=0,
              show_default=True)
@click.option('-b', '--initial-backoff', type=click.INT,
              help='Number of seconds to wait before the first retry. Any subsequent retries will be powers of '
                   'initial-backoff * 2^retry_number', default=2, show_default=True)
@click.option('-B', '--max-backoff', type=click.INT, help='Maximum number of seconds a retry will wait',
              default=600, show_default=True)
def write(client, docs, index_name, refresh, chunk_size, max_chunk_bytes, max_retries, initial_backoff, max_backoff):
    ...
    for actions in _read_actions(docs, max_chunk_bytes):
        for action in actions:
            ...
            action['_type'] = '_doc' if '_type' not in action.keys() else action['_type']

        successful_request_count, errors = bulk(client, actions, chunk_size=chunk_size, raise_on_error=False,
                                                max_chunk_bytes=max_chunk_bytes, max_retries=max_retries,
        ...
        if not ok:
            LOG.error('{} / {} failed documents.'.format(chunk_count - successful_request_count,
                                                         chunk_count))
            for error in errors:
...
def _read_actions(path, max_chunk_bytes):
    def _stripped_lines():
        return [json.loads(s.strip()) for s in f.readlines(max_chunk_bytes) if s.strip() != '']
