...
@click.option('-R', '--remote', type=click.STRING, metavar='NAME',
              help='Remote cluster or hostname as reindex source.')
@click.option('-S', '--socket-timeout', type=click.STRING, default='30s', help='Socket timeout for remote host.',
              show_default=True)
@click.option('-C', '--connection-timeout', type=click.STRING, default='30s', show_default=True,
              help='Connection timeout for remote host.')
@click.option('-w', '--wait', is_flag=True, help='Wait for reindexing to finish, before returning.')
@click.option('-s', '--size', type=click.INT, default=None, help='Upper boundary for count of docs to reindex.',
              show_default=True)
@click.option('-b', '--batch-size', type=click.INT, default=None, help='The batch size to use while indexing.',
              show_default=True)
@click.option('-i', '--slices', type=click.INT, default=0, show_default=True,
...
def start(client, site, source_index, target_index, remote, wait, size, socket_timeout,
          connection_timeout, batch_size, slices):
    ...
    if destination is None:
        ...
        click.confirm('Destination index does not exist and will therefore be created from the '
...
def progress(client, task_id):
    ...
    modified = reindex_status['created'] + \
