...
def _init_dirs(app_dir, log_dir_name):
    ...
    try:
        ...
        click.secho(u'Could not setup application directory: {}'.format(app_dir), err=True, fg=u'red')
...
def _copy_default_app_config(app_dir, config_basename=None, default_config=DEFAULT_CONFIG):
    ...
    if not path.isfile(path.join(app_dir, config_basename)):
        try:
            ...
            click.secho(u'Could not write default config to: {}'.format(app_dir), err=True, fg=u'red')
