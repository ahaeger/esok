...
def test_copy_default_app_config(tmpdir):
    ...
    with conf_path.open() as config_file:
        with open(DEFAULT_CONFIG, u'r') as default_config:
            assert config_file.read() == default_config.read(), u'Config file contents are not the defaults.'
...
@pytest.mark.skipif(getuser() == u'root', reason=u'Test does not work if running as root, like Tingle does.')
