...
def test_no_connection_options(runner):
    ...
    assert clients == ['localhost'], 'Default connection should be used when no connection options are passed.'
...
def test_cluster_with_predefined_connections(user_config_file, runner):
    ...
    assert 'some.es.cluster.example.com' in clients, 'Configured cluster should be part of the clients to connect to.'
    assert 'east-asia-host' in clients, 'Configured cluster should be part of the clients to connect to.'
...
@pytest.mark.usefixtures('mock_clients')
def test_cluster_with_predefined_connections_and_missing_site_option(user_config_file, runner):
    user_config_file.write_text("""
    [cluster:awesome-cluster]
    us = some.es.cluster.example.com
    ap = asia-host
...
def test_cluster_with_invalid_pattern_config(user_config_file, runner):
    ...
    assert r.exit_code == CONFIGURATION_ERROR, 'Exit code should signal a configuration error.'
    assert 'Error' in r.output
...
def test_cluster_with_pattern_config_without_site(user_config_file, runner):
    ...
    assert r.exit_code == 0, 'The command should succeed.'
    assert 'my-favorite-cluster' in clients, 'Specified cluster should be the only client.'


@pytest.mark.usefixtures('mock_clients')
def test_cluster_with_pattern_config_and_site_option_without_site_variable(user_config_file, runner):
    user_config_file.write_text("""
    [general]
    cluster_hostname_pattern = my-{cluster}-cluster
    """)
...
def test_cluster_with_pattern_config_and_default_sites(user_config_file, runner):
    ...
    assert r.exit_code == 0, 'The command should succeed.'
    assert 'my-favorite-site1-cluster' in clients, 'Specified cluster should be among the clients.'
    assert 'my-favorite-site2-cluster' in clients, 'Specified cluster should be among the clients.'
...
@pytest.mark.usefixtures('mock_clients')
def test_cluster_with_pattern_config_and_default_sites_without_site_variable(user_config_file, runner):
    user_config_file.write_text("""
    [general]
    cluster_hostname_pattern = my-{cluster}-cluster
    cluster_pattern_default_sites = site1,site2
...
@pytest.mark.usefixtures('mock_clients')
def test_cluster_with_pattern_config_and_default_sites_and_site_option(user_config_file, runner):
    user_config_file.write_text("""
    [general]
    cluster_hostname_pattern = my-{cluster}-{site}-cluster
    cluster_pattern_default_sites = site1,site2
    ...
    assert r.exit_code == 0, 'The command should succeed.'
    assert ['my-favorite-site3-cluster'] == clients, 'Specified cluster should be the only client.'


@pytest.mark.usefixtures('mock_clients')
def test_cluster_with_pattern_config_predefined_connections_takes_precedence(user_config_file, runner):
    user_config_file.write_text("""
    [general]
    cluster_hostname_pattern = my-{cluster}-{site}-cluster
    cluster_pattern_default_sites = site1,site2
...
@pytest.mark.usefixtures('mock_clients')
def test_cluster_with_pattern_config_and_default_sites_are_stripped_of_whitespace(user_config_file, runner):
    user_config_file.write_text("""
    [general]
    cluster_hostname_pattern = my-{cluster}-{site}-cluster
    cluster_pattern_default_sites = site1, site2
...
def test_include_site_should_pass_site(user_config_file, runner):
    ...
    assert r.exit_code == 0, 'The command should succeed.'
    assert ('host1', 'eu') in clients, 'Configured site-to-host mapping should be preserved.'
    assert ('host2', 'us') in clients, 'Configured site-to-host mapping should be preserved.'
...
def test_progress_is_printed_for_several_clients(user_config_file, runner):
    ...
    assert r.exit_code == 0, 'The command should succeed.'
    assert 'awesome-cluster - eu:\n' in r.output, 'CLI should output which cluster it is connecting to.'
    assert 'awesome-cluster - us:\n' in r.output, 'CLI should output which cluster it is connecting to.'
    assert 'awesome-cluster - ae:\n' in r.output, 'CLI should output which cluster it is connecting to.'
...
def test_no_progress_is_printed_for_only_one_client(user_config_file, runner):
    ...
    assert r.exit_code == 0, 'The command should succeed.'
    assert r.output == '', 'There should be no output when only one cluster is being connected to.'
...
def mock_clients(monkeypatch):
    monkeypatch.setattr('esok.config.connection_options.Elasticsearch',
                        lambda *args, **kwargs: (args, kwargs))
    monkeypatch.setattr('esok.config.connection_options.create_default_context', lambda *args, cafile, **kwargs: cafile)
